...
export default function Navbar() {
  return (
    <nav className="border-base sticky top-0 z-50 flex h-16 items-center justify-between border-b px-4 shadow-default backdrop-blur-xl lg:hidden">
      <MobileMenu />
      <ThemeToggle />
    </nav>
  )
  ...
  return (
    <Dialog.Root open={isMenuOpen} onOpenChange={setMenuOpen}>
      ...
      <Dialog.Portal className="h-screen">
        <Dialog.Overlay className="bg-foreground fixed inset-0 z-10" />
                      ...
                      <Dialog.Close key={page.path} asChild>
                        <Link
                          className={classNames(
                            'transitions-colors rounded-default ml-0.5 px-4 py-2 text-[0.9rem] outline-none duration-200 focus:ring-subtle focus-visible:ring-2',
                            pathname === page.path

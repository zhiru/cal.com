...
export const FormBuilderField = ({
  ...
  const { hidden, placeholder, label, noLabel } = getAndUpdateNormalizedValues(field, t);
...
function getAndUpdateNormalizedValues(field: RhfFormFields[number], t: TFunction) {
  ...
  const label = field.labelAsSafeHtml || field.label || t(field.defaultLabel || "");
  ...
  return { hidden, placeholder, label, noLabel };
}
